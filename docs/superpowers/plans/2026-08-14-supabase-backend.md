...
**Goal:** Remplacer l'authentification localStorage par un vrai backend Supabase avec abonnement Wave manuel et panel admin.

**Architecture:** Site vanilla HTML/JS sur Vercel. Supabase Client importé via CDN ES module. Téléphone converti en email interne `{tel}@eautoecole.sn` pour éviter SMS payants. Sécurité via RLS Supabase.

**Tech Stack:** HTML5, CSS3, JavaScript ES6 modules, Supabase JS v2 (CDN), Supabase Auth + Database + Storage, Vercel
...
- Importer Supabase via CDN : `https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm`
...
- [ ] **Step 1 : Créer un compte et un projet Supabase**
  ...
  - **Region :** choisir `West EU (Ireland)` ou `US East` (le plus proche disponible)
...
- [ ] **Step 2 : Désactiver la confirmation email**
  ...
  Pourquoi : on utilise de faux emails internes `{tel}@eautoecole.sn`, donc aucun email ne peut être confirmé.
...
**Interfaces:**
- Produit : tables `profiles`, `subscriptions`, `payment_requests` avec RLS — utilisées dans toutes les tâches suivantes
...
- [ ] **Step 7 : Vérifier**
  ...
  Dans `Table Editor` : les 3 tables doivent apparaître avec le cadenas RLS activé.
...
- [ ] **Step 2 : Créer les politiques Storage**
  ...
  `Storage` → `payment-screenshots` → `Policies` → `New policy` → "For full customization"
...
- [ ] **Step 3 : Vérifier**
  ...
  `Storage` → `payment-screenshots` → le bucket apparaît, vide, avec les politiques listées.
...
- Produit : `supabase` (client), `phoneToEmail()`, `getSession()`, `getProfile()`, `isAdmin()`, `signUp()`, `signIn()`, `signOut()`, `checkAccess()` — utilisés dans Tasks 5, 6, 7, 8

- [ ] **Step 1 : Créer le fichier supabase.js**

  Créer `/supabase.js` à la racine du projet avec ce contenu (remplacer les valeurs par celles de Task 1 Step 3) :
...
- [ ] **Step 3 : Test rapide dans la console navigateur**
  ...
  Ouvrir n'importe quel fichier HTML du projet dans le navigateur (via `file://` ou un serveur local). Ouvrir la console et taper :
  ...
  Résultat attendu : `Session: null` (personne connecté). Si erreur réseau → vérifier l'URL Supabase.
  ...
  <head>
    ...
    <style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .message.error { background: #fdecea; color: var(--error); display: block; }
      .message.success { background: #eafaf1; color: var(--success); display: block; }
      .contact-admin {
        text-align: center;
        margin-top: 16px;
        font-size: 0.8rem;
  ...
  <body>
    <div class="auth-container">
      ...
      <div id="tab-connexion" class="tab-content active">
        <form id="form-connexion" onsubmit="handleConnexion(event)">
          <div class="form-group">
            ...
            <div class="input-wrapper">
              ...
              <input type="tel" id="login-tel" placeholder="Ex: 762000000" required>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="input-wrapper">
              ...
              <input type="password" id="login-pwd" placeholder="Votre mot de passe" required>
            </div>
          </div>
        ...
        <p class="contact-admin">Mot de passe oublié ? Contactez l'administrateur.</p>
      </div>
        ...
        <form id="form-inscription" onsubmit="handleInscription(event)">
          <div class="form-group">
            ...
            <div class="input-wrapper">
              ...
              <input type="text" id="reg-nom" placeholder="Votre nom et prénom" required>
            </div>
          </div>
          <div class="form-group">
            <label>Numéro de téléphone</label>
            <div class="input-wrapper">
              <i class="fas fa-phone"></i>
              <input type="tel" id="reg-tel" placeholder="Ex: 762000000" required>
            </div>
          </div>
          <div class="form-group">
            <label>Mot de passe</label>
            <div class="input-wrapper">
              <i class="fas fa-lock"></i>
              <input type="password" id="reg-pwd" placeholder="Minimum 8 caractères" minlength="8" required>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <i class="fas fa-lock"></i>
              <input type="password" id="reg-pwd2" placeholder="Répéter le mot de passe" required>
            </div>
          </div>
    ...
    <script type="module">
      import { signIn, signUp, getSession } from '/supabase.js'
      ...
      window.switchTab = function(tab) {
        document.querySelectorAll('.tab-btn').forEach((b, i) => {
          b.classList.toggle('active', (i === 0 && tab === 'connexion') || (i === 1 && tab === 'inscription'))
        })
      ...
      window.handleConnexion = async function(e) {
        ...
        if (error) {
          showMsg('msg-connexion', 'Numéro ou mot de passe incorrect.', 'error')
          btn.disabled = false
      ...
      window.handleInscription = async function(e) {
        ...
        if (pwd !== pwd2) {
          showMsg('msg-inscription', 'Les mots de passe ne correspondent pas.', 'error')
          return
        }
        ...
        if (error) {
          ...
          showMsg('msg-inscription', 'Compte créé ! Vous allez être redirigé...', 'success')
...
- [ ] **Step 2 : Tester auth.html**
  ...
  **Test inscription :**
  ...
  - Vérifier dans Supabase Dashboard → `Authentication` → `Users` : un utilisateur apparaît
  - Vérifier dans `Table Editor` → `profiles` : le profil est créé avec `is_active = false`
  ...
  <head>
    ...
    <style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .header p { color: var(--text-light); font-size: 0.85rem; margin-top: 4px; }
      .card {
        background: var(--card);
        border-radius: 12px;
      ...
      .price-badge .amount { font-size: 2.2rem; font-weight: 700; }
      .price-badge .label { font-size: 0.85rem; opacity: 0.9; margin-top: 4px; }
      ...
      .wave-icon { font-size: 1.8rem; }
      .wave-number { font-size: 1.3rem; font-weight: 700; color: var(--wave); }
      .wave-name { font-size: 0.8rem; color: var(--text-light); }
      ...
      .upload-zone p { font-size: 0.85rem; color: var(--text-light); }
      ...
      .status-pending { background: #fff8e1; border-left: 4px solid var(--warning); }
      .status-rejected { background: #fdecea; border-left: 4px solid var(--error); }
      .status-card h4 { font-size: 0.9rem; margin-bottom: 4px; }
      .status-card p { font-size: 0.82rem; color: var(--text-light); }
      .message {
        padding: 12px;
        border-radius: 8px;
        font-size: 0.85rem;
  ...
  <body>
    <div class="container">
      ...
      <div class="card" id="payment-card">
        ...
        <div class="wave-section">
          ...
          <div class="wave-info">
            ...
            <div>
              ...
              <div class="wave-name">Auto-École Dieynaba</div>
            </div>
          </div>
        </div>
        ...
        <button class="btn-primary" id="btn-submit" onclick="submitPayment()" disabled>
    ...
    <script type="module">
      ...
      async function init() {
        ...
        const profile = await getProfile()
        if (profile?.is_active) { window.location.href = '/index.html'; return }
          ...
          if (req.statut === 'rejected') {
            zone.innerHTML = `
              <div class="status-card status-rejected">
                ...
                <p>${req.note_admin || 'Capture non valide. Veuillez soumettre une nouvelle capture.'}</p>
              </div>`
          }
        }
      }
      ...
      window.submitPayment = async function() {
        ...
        if (uploadError) {
          ...
          document.getElementById('msg-payment').textContent = 'Erreur lors de l\'upload. Réessayez.'
          btn.disabled = false
          btn.innerHTML = '<i class="fas fa-paper-plane"></i> Envoyer ma preuve de paiement'
          return
        }
        ...
        if (dbError) {
          document.getElementById('msg-payment').className = 'message error'
          document.getElementById('msg-payment').textContent = 'Erreur lors de l\'envoi. Réessayez.'
          btn.disabled = false
          btn.innerHTML = '<i class="fas fa-paper-plane"></i> Envoyer ma preuve de paiement'
          return
        }
        ...
        document.getElementById('msg-payment').textContent = 'Envoyé ! Un admin va vérifier votre paiement prochainement.'
        document.getElementById('payment-card').style.display = 'none'
...
- [ ] **Step 2 : Configurer le numéro Wave**
  ...
  Dans `payment.html`, ligne `const WAVE_NUMBER = '77 XXX XX XX'`, remplacer par le vrai numéro Wave de l'auto-école.
...
- [ ] **Step 3 : Tester payment.html**
  ...
  - Vérifier dans Supabase Dashboard → `Table Editor` → `payment_requests` : une ligne avec `statut = 'pending'`
...
**Interfaces:**
- Consomme : `isAdmin()`, `supabase`, `getSession()`, `signOut()` depuis `supabase.js`
  ...
  <head>
    ...
    <style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .page-title { font-size: 1.2rem; font-weight: 700; margin-bottom: 20px; }
      .card {
        background: var(--card);
        border-radius: 12px;
      ...
      td { padding: 14px 16px; font-size: 0.88rem; border-bottom: 1px solid var(--border); }
      ...
      .badge-approved, .badge-active { background: #eafaf1; color: var(--success); }
      .badge-rejected, .badge-inactive { background: #fdecea; color: var(--error); }
      .btn-sm {
        padding: 6px 12px;
        border: none;
        border-radius: 6px;
      ...
      .btn-toggle { background: #e8f4fd; color: #0077b6; }
      .btn-view { background: #f0f0f0; color: var(--text); }
      .empty { text-align: center; padding: 40px; color: var(--text-light); font-size: 0.9rem; }
      .modal-overlay {
        position: fixed;
        inset: 0;
      ...
      .modal img { max-width: 400px; max-height: 70vh; object-fit: contain; border-radius: 8px; }
      ...
      .reject-input:focus { border-color: var(--error); }
      @media (max-width: 768px) {
        .sidebar { width: 60px; }
        .sidebar-logo, .nav-btn span, .btn-logout-side { display: none; }
        .nav-btn { justify-content: center; padding: 16px; }
      }
    </style>
  </head>
  <body>
    <div class="layout">
      <div class="sidebar">
        ...
        <button class="nav-btn active" onclick="showPage('demandes')" id="nav-demandes">
          <i class="fas fa-clock"></i>
          <span>Demandes</span>
          <span class="badge" id="badge-count" style="display:none">0</span>
        </button>
        <button class="nav-btn" onclick="showPage('utilisateurs')" id="nav-utilisateurs">
    ...
    <div class="modal-overlay" id="modal" onclick="closeModal(event)">
      <div class="modal">
        <h3 style="margin-bottom:12px; font-size:0.95rem;">Capture de paiement Wave</h3>
        <img id="modal-img" src="" alt="Capture Wave">
        <input class="reject-input" id="reject-note" type="text" placeholder="Raison du rejet (optionnel)" style="display:none">
        <div class="modal-actions">
          <button class="btn-sm btn-approve" id="btn-modal-approve">✅ Approuver</button>
          <button class="btn-sm btn-reject" id="btn-modal-reject-show">❌ Rejeter</button>
    ...
    <script type="module">
      import { supabase, isAdmin, getSession, signOut } from '/supabase.js'
      ...
      async function init() {
        const session = await getSession()
        if (!session) { window.location.href = '/auth.html'; return }
        if (!(await isAdmin())) { window.location.href = '/index.html'; return }
        await loadDemandes()
      }
      ...
      async function loadDemandes() {
        ...
        if (error || !data?.length) {
          tbody.innerHTML = '<tr><td colspan="5" class="empty">Aucune demande pour l\'instant.</td></tr>'
          return
        }
          ...
          <tr>
            ...
            <td>
              <span class="badge badge-${req.statut}">
                ${req.statut === 'pending' ? '⏳ En attente' : req.statut === 'approved' ? '✅ Approuvé' : '❌ Rejeté'}
              </span>
            </td>
            <td>
      ...
      window.viewCapture = async function(reqId, screenshotPath, userId, statut) {
      ...
      document.getElementById('btn-modal-approve').onclick = async function() {
        ...
        await supabase.from('profiles').update({ is_active: true }).eq('id', currentUserId)
      ...
      document.getElementById('btn-modal-reject-show').onclick = async function() {
...
- [ ] **Step 2 : Tester admin.html (après Task 9 — création admin)**
  ...
  - Cliquer "Approuver" → badge disparaît, vérifier dans Supabase que `profiles.is_active = true`
...
- [ ] **Step 1 : Lire le bloc d'authentification actuel dans index.html**
  ...
  - Le bloc `<script>` qui gère le localStorage auth (login, register, session check)
...
- [ ] **Step 2 : Ajouter l'import Supabase et la vérification de session**
  ...
  En haut du premier `<script>` de `index.html`, ajouter `type="module"` au tag script principal, puis ajouter en tout début :

  ```javascript
  import { checkAccess, signOut, getProfile } from '/supabase.js'
  ...
  **Note :** les IDs `userName`, `loginScreen`, `mainApp` sont à adapter selon les vrais IDs présents dans `index.html`. Lire le fichier avant de modifier.

- [ ] **Step 3 : Remplacer le bouton de déconnexion**

  Trouver la fonction de déconnexion dans `index.html` (ex: `logout()`, `handleLogout()`) et la remplacer par :
...
- [ ] **Step 4 : Supprimer le code localStorage auth**
  ...
  Supprimer ou commenter les blocs suivants (ne pas supprimer la logique UI non-auth) :
...
- [ ] **Step 1 : Créer le compte admin**
  ...
  **b) Ajouter le rôle admin dans app_metadata :**
  Dans `Authentication` → `Users` → cliquer sur l'utilisateur créé → modifier `app_metadata` :
  ```json
  {
    "role": "admin"
  }
...
- [ ] **Step 2 : Tester le flux complet utilisateur → admin**
  ...
  **Flux 1 — Nouveau utilisateur :**
  ...
  5. Vérifier dans `Table Editor` → `payment_requests` : ligne avec `statut = 'pending'`
...
- [ ] **Step 3 : Déployer sur Vercel**
  ...
  Vérifier le déploiement sur Vercel → tester les 3 flux sur l'URL de production.
